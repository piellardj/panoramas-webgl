...
(function (Page) {
    ...
    (function (Helpers) {
        ...
        (function (URL) {
            ...
            var randomState = { randInt: Math.random() };
            var pushedACustomState = false;
            function updateUrl(newUrl) {
                if (!pushedACustomState) {
                    window.history.pushState(randomState, document.title, newUrl);
                    pushedACustomState = true;
                }
                else {
                    window.history.replaceState(randomState, document.title, newUrl);
                }
            }
...
Page.Canvas.setMaxSize(768,512);